...
properties. The elimination principle, {lean type:="{motive : Weekday → Sort u} → (sunday : motive Weekday.sunday) → (monday : motive Weekday.monday) → (tuesday : motive Weekday.tuesday) → (wednesday : motive Weekday.wednesday) → (thursday : motive Weekday.thursday) → (friday : motive Weekday.friday) → (saturday : motive Weekday.saturday) → (t : Weekday) → motive t"}`Weekday.rec`, is defined
...
Actually, Lean compiles the {kw}`match` expressions using a definition based on
{leanRef}`Eq.rec`.
...
{leanRef}`Eq.rec`, the eliminator for the inductively defined equality
