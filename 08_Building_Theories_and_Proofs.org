...
-- BEGIN
persistent attribute morphism.mor [coercion]
-- END
#+END_SRC
...
can temporarily alter the annotation with the =attribute= command.
These changes are effective only in the
