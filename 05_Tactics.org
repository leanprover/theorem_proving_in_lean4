...
#+END_SRC
In a situation like this, when there is only a single tactic step
involved, you can use the =by= keyword instead of using a
=begin...end= block.
#+BEGIN_SRC lean
theorem test (p q : Prop) (hp : p) (hq : q) : p ∧ q ∧ p :=
...
# In a tactic block, Lean interprets =have p, from t₁,
...
=assert=s below open goals for =p= and =q ∨ r=, respectively, and then
add =hp : p= and =hqr : q ∨ r= to the context for subsequent use.
#+BEGIN_SRC lean
example (p q r : Prop) : p ∧ (q ∨ r) → (p ∧ q) ∨ (p ∧ r) :=
begin
  intro h,
...
#+END_SRC
Here type types can be omitted, so we can write =note hp := h.left=
...
tactic is =rewrite t=, where =t= is a term whose conclusion is an
