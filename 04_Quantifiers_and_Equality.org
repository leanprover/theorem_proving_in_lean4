...
example (A B : Type) (f : A → B) (a : A) : (λ x, f x) a = f a := eq.refl _
...
open nat prod
-- BEGIN
example (A B : Type) (f : A → B) (a : A) : (λ x, f x) a = f a := rfl
...
#+END_SRC
  # (unreadable) solutions to some of these examples:
  # example : (∃ x, p x ∨ q x) ↔ (∃ x, p x) ∨ (∃ x, q x) :=
  # iff.intro (λ L, obtain a H, from L, or.elim H (λ LL, or.inl (exists.intro a LL))
  #                                              (λ LR, or.inr (exists.intro a LR)))
  #           (λ R, or.elim R (λ RL, obtain a H, from RL, exists.intro a (or.inl H))
  #                          (λ RR, obtain a H, from RR, exists.intro a (or.inr
  # example : (¬∀ x, p x) ↔ (∃ x, ¬p x) :=
  # iff.intro (λ L, by_contradiction (λ NR, L (λ a, by_contradiction (λ H, NR (exists.intro a H)))))
  #           (λ R LH, obtain a np, from R, np (LH a))
  # example (a : A) : (∃ x, p x → r) ↔ (∀ x, p x) → r :=
  # iff.intro (λ L RH, obtain a H, from L, H (RH a))
  #           (λ R, or.elim (em r)
  #             (λ rt, exists.intro a (λ x, rt))
  #             (λ rf, by_contradiction (λ NL, rf (R (λ b, by_contradiction
  #               (λ np, NL (exists.intro b (λ pt, absurd pt np))))))))
