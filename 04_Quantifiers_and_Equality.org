...
y) * (x - y)=, using the theorems =mul_sub_right_distrib= and
=mul_sub_left_distrib= in the module [[https://github.com/leanprover/lean/blob/master/library/data/nat/sub.lean][data.nat.sub]].
