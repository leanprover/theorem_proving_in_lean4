...
# TODO: when we have something like the Lean 2 `p` notation, we 
# can use that in examples below
...
When the goal can be inferred, we can also ask Lean to fill in the
...
As with the anonymous =have=, when you use =suppose= to introduce an
assumption, Lean can also find it later with the =assumption= tactic.
#+BEGIN_SRC lean
variable f : ℕ → ℕ
premise h : ∀ x : ℕ, f x ≤ f (x + 1)
...
suppose f 0 ≥ f 1,
suppose f 1 ≥ f 2,
have f 0 ≥ f 2, from nat.le_trans this (by assumption),
have f 0 ≤ f 2, from nat.le_trans (h 0) (h 1),
show f 0 = f 2, from nat.le_antisymm this (by assumption)
-- END
#+END_SRC
Notice that =le_antisymm= is the assertion that if =a ≤ b= and =b ≤ a=
then =a = b=, and =a ≥ b= is definitionally equal to =b ≤ a=.

# TODO: add this sort of stuff if / when it becomes available, or
#   delete if we decide not to use it
