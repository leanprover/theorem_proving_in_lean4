...
the expression =B= may depend on a variable =x : A=. Then the type of
=Πx : A, B= is an element of =Type.{imax i j}=, where =imax i j= is
the maximum of =i= and =j= if =j= is not 0, and 0 otherwise. 
...
/impredicative/. The key feature distinguishing Lean's library for
homotopy type theory from the standard library is the absence of
=Prop=, making that theory /predicative/.
