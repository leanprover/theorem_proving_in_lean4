...
#+END_SRC
  ...
  # iff.intro (λL, obtain a H, from L, or.elim H (λLL, or.inl (exists.intro a LL))
  #                                              (λLR, or.inr (exists.intro a LR)))
  #           (λR, or.elim R (λRL, obtain a H, from RL, exists.intro a (or.inl H))
  #                          (λRR, obtain a H, from RR, exists.intro a (or.inr
  # example : (¬∀x, p x) ↔ (∃x, ¬p x) :=
  # iff.intro (λL, by_contradiction (λNR, L (λa, by_contradiction (λH, NR (exists.intro a H)))))
  #           (λR LH, obtain a np, from R, np (LH a))
  # example (a : A) : (∃x, p x → r) ↔ (∀x, p x) → r :=
  # iff.intro (λL RH, obtain a H, from L, H (RH a))
  #           (λR, or.elim (em r)
  #             (λrt, exists.intro a (λx, rt))
  #             (λrf, by_contradiction (λNL, rf (R (λb, by_contradiction
  #               (λnp, NL (exists.intro b (λpt, absurd pt np))))))))
