...
processing a file. Moreover, for efficiency purposes,
Lean treats theorems as axiomatic constants within the file that they
are defined. This makes it possible to process and check theorems in
...
to for Lean to use the theorem itself, rather than its axiomatic
surrogate.
#+BEGIN_SRC lean
constants p q : Prop
