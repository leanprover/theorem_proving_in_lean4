...
=Prop= has some special features, but like the other type universes,
...
show r, from h₁ (h₂ h₃)
#+END_SRC
As a theorem of propositional logic, what does =t2= say? It is often
useful to use numeric unicode subscripts, entered as =\0=, =\1=, =\2=,
...
situations like these, when the relevant type is an inductive type and
can be inferred from the context. In particular, we can often write
=⟨hp, hq⟩= instead of =and.intro hp hq=:
#+BEGIN_SRC lean
variables p q : Prop
variables  (hp : p) (hq : q)
