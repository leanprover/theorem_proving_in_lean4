...
Indeed, if =p : Prop= is any proposition, Lean's /standard kernel/ treats any two
...
In the previous paragraph, we use the term /standard kernel/ because
Lean also supports an alternative /proof relevant kernel/.
The alternative kernel will be described later when we discuss how
[[http://homotopytypetheory.org/][Homotopy Type Theory]] can be developed using Lean.
...
are always treated as "opaque" definitions, and Lean never tries to
"unfold" the definition and "see" the proof.
The point is that later definitions and theorems should not care what the proof is;
by the assumption of proof irrelevance, they are all treated the same.
Note that =theorems= are essentially =opaque definitions=. There is only a
small difference: in Lean, opaque definitions are treated as transparent in the
