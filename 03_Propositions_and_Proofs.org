...
=theorem= is usually treated as an /opaque/ definition, and Lean never
...
-- END
#+END_SRC
The type of =t1= is now =Π p q : Prop, p → q → p=. We can read this as
...
If you are not used to thinking constructively, it make take some time
