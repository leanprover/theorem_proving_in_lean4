...
-- We can use use bounded quantifiers to implement computable functions.
-- The function (is_constant_range a n) returns tt iff it evaluates
-- to the same value for all 0 <= i < n
open bool
definition is_constant_range (a : nat → nat) (n : nat) : bool :=
if ∀ i, i < n → a i = a 0 then tt else ff
