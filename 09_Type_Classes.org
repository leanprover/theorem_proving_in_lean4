...
class/. (Currently, Lean imposes the added restriction that the family
has to be a family of /inductive types/.) Then we can declare
particular elements of a type class to be /instances/. These provide
hints to the elaborator: any time the elaborator is looking for an
element of a type class, it can consult a table of declared instances
