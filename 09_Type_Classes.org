...
#+END_SRC
In contrast, =arbitrary nat = nat.zero= is provable in the module where
...
We say it is the dependent if-then-else expression. It is defined as follows:
#+BEGIN_SRC lean
namespace hide
...
=Hnc : ¬ c= in the "else" branch.
To make =dite= more convenient ot use, Lean provides the
syntactic sugar =if h : c then t else e= for =dite c (λ h : c, t) (λ h : ¬ c, e)=.
...
check (λx : nat, (1 : nat)) + (λx, (2 : nat))   -- ℕ → ℕ
eval (λx : nat, (1 : nat)) + (λx, (2 : nat))    -- λ (x : ℕ), 3
-- END
#+END_SRC
...
assuming =P= is, and that type class inference can make use of that fact.
