...
check i + ↑n + ↑m
check i + ↑(n + m)
/- The following doesn't work because we process i, the expected type is nat,
   but we don't have a coercion from int to nat. -/
-- check n + i
/- The operator ↑ can be used to save the day. In instructs Lean, that it
   must use a coercion around n. -/
check ↑n + i
-- END
#+END_SRC
