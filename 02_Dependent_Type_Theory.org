...
are considered "the same" by the underlying logical framework, and
...
Pi types. The type =Σx : A, B x= denotes the type of pairs =dpair a b=
where =a : A= and =b : B a=. Just as Pi types =Πx : A, B x= generalize
the notion of a function type =A → B=, Sigma types =Σx : A, B x=
generalize the cartesian product =A × B=: in the expression =dpair a
b=, the type of the second element of the pair, =b : B a=, depends on
