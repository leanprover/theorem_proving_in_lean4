...
Pi types. The type =Σx : A, B x= denotes the type of pairs =⟨a, b⟩=
where =a : A= and =b : B a=. Just as Pi types =Πx : A, B x= generalize
the notion of a function type =A → B=, Sigma types =Σx : A, B x=
generalize the cartesian product =A × B=: in the expression =⟨a, b⟩=,
...
check ⟨a, b⟩
check sigma.mk a b -- ⟨a, b⟩ is notation for sigma.mk a b
check pr1 ⟨a, b⟩
check pr2 ⟨a, b⟩
