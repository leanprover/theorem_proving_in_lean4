...
def config : RenderConfig where
  ...
  linkTargets := fun st remoteContent => st.localTargets ++ remoteContent.remoteTargets
  remoteConfigFile := some "verso-sources-empty.json"

def main := manualMain (%doc TPiL) (options := []) (config := config)
