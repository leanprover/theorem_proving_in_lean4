...
到目前为止，你已经见过一些在 Lean 中定义对象和函数的方法。在本章中，我们还将开始说明如何用依赖类型论的语言书写数学断言和证明。
...
要证明关于依赖类型论语言中所定义对象的断言，一种策略是在定义语言之上再叠加一套断言语言和一套证明语言。但没有理由以这种方式增殖语言：依赖类型论灵活而富有表达力，我们没有理由不能在同一个一般框架中表示断言和证明。
...
此外，一旦作出这种认同，蕴含的规则就表明，我们可以在 {lean}`Implies p q` 与 {lean}`p → q` 之间来回转换。换言之，命题 {lean}`p` 与 {lean}`q` 之间的蕴含，对应于拥有一个函数，它把 {lean}`p` 的任意元素映到 {lean}`q` 的一个元素。因此，引入联结词 {lean}`Implies` 完全是多余的：我们可以使用依赖类型论中通常的函数空间构造子 {lean}`p → q` 作为我们的蕴含概念。
...
事实上，如果 {lean}`p : Prop` 是任意命题，Lean 的内核会把任意两个元素 {lean}`t1 t2 : p` 视为定义相等，这与它把 {lit}`(fun x => t) s` 和 {lit}`t[s/x]` 视为定义相等非常类似。这称为 {deftech}_proof irrelevance_（证明无关性），并且与上一段的解释一致。它意味着，即使我们可以把证明 {lean}`t : p` 当作依赖类型论语言中的普通对象来处理，它们除了 {lean}`p` 为真这一事实之外不携带任何信息。

我们提出的关于 {tech}[propositions-as-types]（命题即类型）范式的两种理解方式，在根本上有所不同。从构造性的观点看，证明是抽象的数学对象，由依赖类型论中合适的表达式来 _指称_。相反，如果我们按照上面所描述的编码技巧来思考，那么表达式本身并不指称任何有趣的东西。真正起作用的是：我们能够把它们写下来，并检查它们是良类型的；这一事实保证了相关命题为真。换言之，表达式 _本身_ 就是证明。
...
无论如何，真正重要的是底线。要在依赖类型论语言中形式化地表达一个数学断言，我们需要给出一个项 {lean}`p : Prop`。要 _证明_ 该断言，我们需要给出一个项 {lean}`t : p`。Lean 作为证明助手的任务，是帮助我们构造这样的项 {lean}`t`，并验证它形式良好且具有正确的类型。
