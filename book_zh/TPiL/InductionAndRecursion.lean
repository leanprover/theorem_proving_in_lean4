...
Lean 还可以处理更复杂的模式匹配形式，其中依赖类型的参数会对
各种情形施加额外约束。这类 _依赖模式匹配_ 的例子将在
{ref "dependent-pattern-matching"}[依赖模式匹配] 一节中讨论。
...
{ref "dependent-pattern-matching"}[依赖模式匹配] 一节中，我们将看到，
为了使表达式通过类型检查，模式中的某些显式项会被强制为特定形式，
尽管它们并不参与模式匹配。因此它们被称为“{deftech}[inaccessible patterns]”
（不可访问模式）。不过在介绍 {ref "dependent-pattern-matching"}[依赖模式匹配]
之前，我们还不需要使用这种不可访问模式。
...
以及一个证明，说明每个递归应用相对于这个关系都是递减的。依赖类型论强大到
...
有时，依赖匹配模式中的某个参数对于定义本身并非必要，但仍必须包含它，
...
:::leanFirst
不可访问模式可用于澄清并控制使用依赖模式匹配的定义。考虑函数
...
4. 仿照 {ref "dependent-pattern-matching"}[依赖模式匹配] 一节中的例子，
