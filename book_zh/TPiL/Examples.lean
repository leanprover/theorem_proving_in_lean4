...
private def exeSuffix : String :=
  if System.Platform.isWindows then ".exe" else ""

private def subversoExe (name : String) : System.FilePath :=
  projectDir / ".lake" / "packages" / "subverso" / ".lake" / "build" / "bin" / (name ++ exeSuffix)
...
def Helper.fromModule (setup : String) : IO Helper := do
  ...
  let args := #["run", "--install", toolchain, "lake", "env", (subversoExe "subverso-helper").toString, mod]
