...
我们已经看到，Lean 的基础系统包含归纳类型。此外，我们还注意到一个引人注目的事实：仅凭类型宇宙、依值箭头类型和归纳类型，就可以构造起相当宏大的数学体系；其余一切都由此而来。Lean 标准库包含许多归纳类型的实例（例如 {lean}`Nat`、{lean}`Prod`、{lean}`List`），甚至逻辑联结词也是用归纳类型定义的。

回忆一下，只包含一个构造子的非递归归纳类型称为 _结构_ 或 _记录_。积类型是一种结构，依值积（Sigma）类型也是如此。一般而言，每当我们定义一个结构 {lit}`S` 时，通常也会定义 _投影_ 函数，使我们能够“析构” {lit}`S` 的每个实例，并取回存储在其字段中的值。函数 {lean}`Prod.fst` 和 {lean}`Prod.snd` 分别返回一个有序对的第一、第二个元素，它们就是这类投影的例子。
