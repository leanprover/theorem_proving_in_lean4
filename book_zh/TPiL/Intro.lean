...
一种 _依值类型论_ 的版本；它强大到足以证明几乎任何传统数学定理，也具有足够的表达力，能以自然的方式完成这些证明。更具体地说，
Lean 基于一种被称为带归纳类型的构造演算的系统。Lean 不仅能够在依值类型论中定义数学对象、表达数学断言，还可以作为书写证明的语言使用。

由于完全详细的公理化证明极其复杂，定理证明的挑战在于让计算机尽可能填补更多细节。你将在 {ref "dependent-type-theory"}[依值类型论]
